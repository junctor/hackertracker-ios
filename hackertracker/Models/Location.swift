...
struct Schedule: Codable {
    ...
    init?(dictionary: [String: Any]) {
        ...
        begin = dfu.locationTimeFormatter.date(from: dictionary["begin"] as? String ?? tmpDate) ?? Date()
        if let beginTimestamp = dictionary["begin"] as? Timestamp {
            begin = beginTimestamp.dateValue()
        }
        end = dfu.locationTimeFormatter.date(from: dictionary["end"] as? String ?? tmpDate) ?? Date()
